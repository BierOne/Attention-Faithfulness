...
metrics_type = {
    ...
    'Multi_Label' : calc_metrics_qa 
}

class Trainer() :
    def __init__(self, dataset, config, _type="Single_Label") :
        Model = BC.Model
        self.model = Model(config, pre_embed=dataset.vec.embeddings, metrics_type=_type)
...
class Evaluator() :
    def __init__(self, dataset, dirname, _type='Single_Label') :
        Model = BC.Model
        self.model = Model.init_from_config(dirname, metrics_type=_type)
        self.model.dirname = dirname
        self.metric_type = _type
    ...
    def replace_and_run_experiment(self, dataset_name, model_name, test_data, exp="att", delta_type="att_mask", exp_metric="AUCTP", mask_size=1,
        ...
        dir_path = './data/bc'
        save_scores_path = os.path.join(dir_path, dataset_name, model_name, exp)
        ...
        if load_perturbation_scores and exists(os.path.join(save_scores_path, file_name)):
            ...
            preds, new_preds, weights, top_ans_deltas, all_pred_deltas, mask_ratios = \
                self.model.replace_and_run(test_data.X, exp, delta_type,
