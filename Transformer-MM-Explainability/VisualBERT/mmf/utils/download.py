...
def download_resources(resources, download_path, version):
    ...
    if not is_built:
        ...
        for resource in resources:
            resource_path = os.path.join(download_path, resource.file_name)
            resource_name = resource_path.split('/')[-1].split('.')[0]
            resource_dir = '/'.join(resource_path.split('/')[:-1]) + "/" + resource_name + ".lmdb"
            if os.path.exists(resource_path) or os.path.exists(resource_dir):
                print(f"Skipping download {resource_path}")
                continue
