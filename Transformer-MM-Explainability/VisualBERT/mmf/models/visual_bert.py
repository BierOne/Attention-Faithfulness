...
class VisualBERTForClassification(nn.Module):
    ...
    def forward(
        ...
        if self.pooler_strategy == "vqa":
            ...
            # This line is to generate explanation (batch_size == 1)
            # pooled_output = self.vqa_pooler(sequence_output, 1, torch.tensor(index_to_gather, device=sequence_output.device))

        elif self.pooler_strategy == "gqa":
            # In GQA pooling strategy, we use representation from first token
            index_to_gather = torch.zeros_like(input_mask.sum(1)).long()
            pooled_output = self.vqa_pooler(sequence_output, 1, torch.tensor(index_to_gather, device=sequence_output.device))
            pooled_output = self.bert.pooler.dense(pooled_output)
...
class VisualBERT(BaseModel):
    ...
    def forward(self, sample_list: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        if "visual_attention_mask" in sample_list:
            sample_list["attention_mask"][:, max_text_len:] *=  sample_list["visual_attention_mask"]
            # print(sample_list["attention_mask"][:, max_text_len:])
            # print(sample_list["attention_mask"][:, max_text_len:].shape)
