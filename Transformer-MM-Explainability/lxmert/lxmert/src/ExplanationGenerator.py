...
class GeneratorBaselines:
    ...
    def grad_mul_att(self, cam, grad):
        cam = cam.reshape(-1, cam.shape[-2], cam.shape[-1])
        grad = grad.reshape(-1, grad.shape[-2], grad.shape[-1]) # [att_heads, num_tokens, num_tokens]
        # grad = grad.mean(dim=[1, 2], keepdim=True)
        cam = (cam * grad).mean(0) # [N_tokens, num_tokens]
        return cam


    def generate_attn_grad(self, input, index=None, method_name="att_grad"):
        output = self.model_usage.forward(input).question_answering_score
        model = self.model_usage.model
        ...
        grad_t_i = blk.visual_attention.att.get_attn_gradients().detach()
        cam_t_i = blk.visual_attention.att.get_attn().detach()
        cam_t_i = self.grad_mul_att(cam_t_i, grad_t_i)
        # self.R_t_i = torch.matmul(self.R_t_t.t(), torch.matmul(cam_t_i, self.R_i_i))
        self.R_t_i = cam_t_i
        ...
        grad = blk.lang_self_att.self.get_attn_gradients().detach()
        cam = blk.lang_self_att.self.get_attn().detach()
        self.R_t_t = self.grad_mul_att(cam, grad)
    ...
    def generate_input_grad(self, input, index=None, method_name="input_grad"):
        self.R_t_i = self.model_usage.forward_ig(input, folds=1)
        return torch.zeros(1,1), self.R_t_i



    def generate_ig(self, input, index=None, method_name="ig"):
        self.R_t_i = self.model_usage.forward_ig(input)
        return torch.zeros(1,1), self.R_t_i


    def generate_att_norm(self, input, index=None, method_name="att_norm"):
        # Note that this calculation is quite slow
        output = self.model_usage.forward(input, save_values=True).question_answering_score
        model = self.model_usage.model
        # last cross attention + self- attention layer
        blk = model.lxmert.encoder.x_layers[-1]
        # cross attn cam will be the one used for the R_t_i matrix
        self.R_t_i = blk.visual_attention.get_att_norm().detach()
        self.R_t_i = self.R_t_i.mean(0)
